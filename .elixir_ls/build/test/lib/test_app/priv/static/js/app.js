...
var channel = socket.channel("example", {});

channel.join().receive("ok", function (resp) {
  console.log("ok, join", resp);
}).receive("error", function (resp) {
  console.log("Unable to join", resp);
});

document.getElementById("inn_check_btn").addEventListener("click", function (e) {
  var now_date = '[' + new Date().getDate() + '.' + new Date().getMonth() + '.' + new Date().getFullYear() + ' ' + new Date().getHours() + ':' + new Date().getMinutes() + ']';
  var payload = { body: document.getElementById("inn_body").value, date: now_date };
  channel.push("new_inn", payload).receive("error", function (e) {
    return console.log(e);
  });
});

channel.on("new_inn", function (resp) {
  gen_row(resp);
});
